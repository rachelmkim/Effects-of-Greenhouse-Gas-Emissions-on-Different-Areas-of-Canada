...
def read_ghg_emissions_for_maps(filename: str) -> Dict[Any, List[float]]:
    """ EDIT DOCSTRING !!! Read and return the csv of the greenhouse gases dataset as a nested list, with the inner list
    ...
    with open(filename) as file:
        ...
        data_so_far = {}
        for row in reader:
            if int(row[0]) >= 1999 and row[3] == 'TOTAL':
...
def read_daily_mean_temps_one_file(filename: str) -> Dict[int, List[float]]:
    ...
    # check that the data contains the appropriate range i.e. from January 1999 to December 2018.
...
def convert_data_to_list(lines: List[str]) -> Tuple[List[List[float]], bool, bool]:
    ...
    the start date of the range (January 1999) is in the list, and a boolean that represents whether
    ...
    for line in lines:
        if 1999 <= int(line[1:5]) <= 2018:
            ...
            if line_list[0] == 1999 and line_list[1] == 1:
...
def make_data_dictionary(data_as_list: List[List[float]]) -> Dict[int, List[float]]:
    ...
    leap_years_since_1999 = [2000, 2004, 2008, 2012, 2016]
    ...
    for row in data_as_list:
        ...
        elif month == 2 and current_year in leap_years_since_1999:
            for i in range(2, len(row) - 2):
                list.append(data_as_dictionary_so_far[current_year], row[i])
        elif month == 2 and current_year not in leap_years_since_1999:
...
def substitute_outliers(data_as_dictionary: Dict[int, List[float]]) -> Dict[int, List[float]]:
    ...
    Preconditions:
        - all(1999 <= year <= 2018 for year in data_as_dictionary)
